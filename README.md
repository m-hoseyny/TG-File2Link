...
After setting up the project, configure the landing page with your bot's username:

```bash
# Replace 'your_bot_name' with your actual bot's username (without @)
sed -i 's/your_bot_username/your_bot_name/g' index.html
```

For example, if your bot's username is @FileToLinkBot:
```bash
sed -i 's/your_bot_username/FileToLinkBot/g' index.html
```
